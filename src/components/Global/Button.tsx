...
export default function Button({
  ...
  const variants = {
    ...
    fill: {
      ...
      color: color === 'dark' ? 'white' : colors.dark,
    },
  };

  const variantStyles = variants[variant];

  const otherStyles = clsx(
    sizes[size],
    bold && 'font-medium',
  ...
  return (
    ...
    >
      <div
        ...
        className={`text-center flex items-center justify-center gap-2 ${className}`}
      >
        {children}
      </div>
